...
contains
  ...
  subroutine  smk_pr_driver(dates,fire_flux,fire_area,lat,lon,&
  ...
  integer :: m1,m2,m3,ia,iz,ja,jz,ibcon,mynum,i,j,k,iveg_ag,&
             k_CO_smold,k_PM25_smold,imm,k1,k2,kmt,kk
  integer  :: sd,ud,gd
  integer  :: w, h, wrf_i, wrf_j, gridm, gridt,id,z
  integer, parameter :: jdim=4
  integer ishape(jdim)
  integer ishape2(jdim)
  integer :: varid, itime, ix
  integer :: nlines, res, line_no, dates,fdate
  real    :: burnt_area,STD_burnt_area,dz_flam,rhodzi,heat_input,fire_flux,fire_area
  real    :: ptop
  real dx, dy
  real, dimension(2)    :: ztopmax
  real, dimension(sd) :: col_theta,col_qvapor,col_densi,exner_func,mid_hgt
  real, dimension(sd) :: pot,densi,exner,qvapor,midhgt
  real, dimension(gd) :: col_geoph, geoph
  real, dimension(ud) :: col_u,col_v,uwind,vwind
  real, allocatable, dimension(:) :: tmpcon,dncon,picon,rvcon,zcon,zzcon,ucon,vcon
  character(len=20)    :: outfile
  character(len=36)    :: sys_com
  character(5) x1
  real lat, lon, flat, flon


  !Initial settings

  ia=1;iz=1;ja=1;jz=1; m1=gd
  ...
  !Set our fire inputs to the variables passed on by the parent program
  !run_plume_model.f90. Also pass on the fire time, lat and lon, which will be
  !outputted with the final results.

  heat_input = fire_flux           ! fire heat flux (kW/m^2)
  burnt_area = fire_area           ! fire area (m^2)
  flat = lat
  flon = lon
  fdate = dates
  ...
  write(6,*) "         "
  write(6,*) "         "
  write(6,*) "Working on fire with properties of:"
  write(6,*) "Fire lat / lon / date of fire  / area  / heat flux"
  write(6,*) flat,flon,fdate,burnt_area,heat_input
  write(6,*) "         "
  write(6,*) "         "


  !Ingest model environmental conditions within the Freitas model...
  ...
  call get_env_model(1,m1-1,kmt,gridm,gridt,pot,densi,exner,qvapor,midhgt,geoph,uwind,vwind)


  !Determine fire heat flux based on veg type and create plume (not needed for now..)
  iveg_ag=1
  imm=1
  ...
  write(6,*) "Get fire properties..."

  call get_fire_properties(imm,iveg_ag,burnt_area,heat_input)


  !------  generate plume rise    ------

  write(6,*) "Computing the smoke plume rise..."
  ...
  !Return plume top to main program
...
subroutine get_env_sounding(k1,k2,kmt)
...
! if(use_sound == 1) then
 print*,' reading file rj.2002092718.C.txt'
 open(13,file='./rj.2002092718.C.txt') !wet case
 ...
 do i=1,1000,5
    ...
    zcon(k)  =xm(4,i)-topo
    prcon(k) =xm(5,i)*100.
    tmpcon(k)=xm(6,i)+273.15
    urcon(k) =xm(7,i)
    tdcon(k) =xm(8,i)
    thtcon(k)=xm(11,i)
    rvcon(k) =xm(12,i)
    ucon(k)= - xm(10,i) *sin(3.1416* xm(9,i)/180.)
    vcon(k)= - xm(10,i) *cos(3.1416* xm(9,i)/180.)
    ! print*,k,i,rvcon(k)
 enddo

! write(6,*) (thtcon(k), k=1, 200)
! write(6,*) (zcon(k), k=1, 200)
...
13 continue
 kmt=k
 !print*,'kmt=',kmt,znz
 nk=200  ! number of vertical levels of the sounding
 !k1=1
...
end subroutine get_env_sounding

!------------------------------------------------------------

subroutine get_env_model(k1,k2,kmt,gridm,gridt,pot,densi,exner,qvapor,&
           midhgt,geoph,uwind,vwind)

!use plumegen_coms
!use rconstants
implicit none
 integer :: k2,k,kcon,klcl,kmt,nk,nkmid,i,kk,use_sound,z,gridm,gridt
 integer, intent(in) :: k1
 real :: znz,themax,tlll,plll,rlll,zlll,dzdd,dzlll,tlcl,plcl,dzlcl,dummy
 integer :: n_setgrid = 0
 real, dimension(gridm)    :: pot
 real, dimension(gridm)    :: qvapor
 real, dimension(gridm)    :: press
 real, dimension(gridm)    :: densi
 real, dimension(gridm)    :: uwind
 real, dimension(gridm)    :: vwind
 real, dimension(gridm)    :: exner
 real, dimension(gridm)    :: midhgt
 real, dimension(gridt)    :: geoph


! Set up Freitas model grid...
...
! Gridcell midpoint variables. do z = 1,gridm ! z-direction of the host model

  do z=1,gridm

     thtcon(z) = pot(z)    !fill with the potential air temperature (K, theta) of the host model
     dncon(z) = densi(z)   !fill with the dry air density (kg/m^3) (basic state)of the host model
     picon(z) = exner(z)   !fill with the exner function (exner = cp * (press(i,j,k)/p00)**rocp)
!                            !of the host model
     rvcon(z) = qvapor(z)  !fill with the water vapor mixing ratio (kg/kg) of the host model
     zcon(z) = midhgt(z)   !fill with the cartesian height (meters) of the termodynamic grid point
!                            !(half-levels) above local surface
     ucon(z) = uwind(z)    !fill with the zonal wind (m/s) of the host model
     vcon(z) = vwind(z)    !fill with the meridional wind (m/s) of the host model

  end do

! Height of dynamic level. do z = 1,gridt ! z-direction of the host model

  do z=1,gridt
     zzcon(z) = geoph(z)   !fill with the cartesian height (meters) of the dynamic grid point
  end do
 ...
 !call htint(# of wrf vert levels, variable, variable botton, variable top, 200, model level)
...
! write(6,*) (zt(k),k=1,kmt)
! write(6,*) (upe(k),k=1,kmt)
...
subroutine get_fire_properties(imm,iveg_ag,burnt_area,heat_input)
!use plumegen_coms
implicit none
integer ::  moist,  i,  icount,imm,iveg_ag
real::   bfract,  effload,  heat,  hinc ,burnt_area,heat_input
...
real ::  vc, g,  r,  cp,  eps,  &
         ...
         ztopmax, wmax, rmaxtime, es, esat,ESAT_PR, heat,dt_save
!
integer :: imm
...
!
!
tstpf = 30.0  	!- timestep factor (was 2 orginally) DVM 1/10/2017
...
!
!  If we are on the final output for the model simulation, save the final
!  vertical plume output to a seperate file with the final plume
!  characteristics DVM 12/1/2016

   IF(finalout == 1) then
      WRITE (5, 400) zt(KO)/1000., PEA, W (KO), BTMP, ETMP, VAP1, &
       VAP2, GPKC, GPKH, GPKI, rbuoy(KO)*100.
   endif
